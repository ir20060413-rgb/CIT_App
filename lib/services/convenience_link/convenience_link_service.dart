...
class ConvenienceLinkService {
  ...
  /// 旧形式のキーを生成（移行用）
  static String _getLegacyUserKey(String emailAddress) =>
      '$_keyPrefix${emailAddress.split('@').first}';

  /// 旧形式データからの移行
  static Future<List<ConvenienceLink>?> _migrateLegacyData(
    String userId,
    String? userEmail,
  ) async {
      ...
      if (legacyJsonString != null && legacyJsonString.isNotEmpty) {
        ...
        final links =
            jsonList
                .map(
                  (json) =>
                      ConvenienceLink.fromJson(json as Map<String, dynamic>),
                )
                .toList();
  ...
  /// ユーザーの便利リンクを取得
  static Future<List<ConvenienceLink>> getUserLinks(
    String userId, {
    String? userEmail,
  }) async {
    try {
  ...
  /// ユーザーの便利リンクを保存
  static Future<void> saveUserLinks(
    String userId,
    List<ConvenienceLink> links,
  ) async {
    try {
      final prefs = await SharedPreferences.getInstance();
      final key = _getUserKey(userId);
  ...
  static Future<void> addLink(String userId, ConvenienceLink link) async {
    ...
    final maxOrder =
        links.isEmpty
            ? 0
            : links.map((l) => l.order).reduce((a, b) => a > b ? a : b);
  ...
  /// リンクの順序を更新
  static Future<void> reorderLinks(
    String userId,
    List<ConvenienceLink> reorderedLinks,
  ) async {
    // 新しいorder値を設定
  ...
  static Future<void> _updateLinksToNewOrderInternal(
    String userId,
    List<ConvenienceLink> links,
  ) async {
    try {
      ...
      for (final link in links) {
        if (removedPresetIds.contains(link.id) ||
            removedPresetTitles.contains(link.title)) {
          continue;
        }
      ...
      for (final presetLink in presetLinks) {
        ...
        if (!existingIds.contains(presetLink.id) &&
            !existingTitles.contains(presetLink.title)) {
          updatedLinks.add(
            presetLink.copyWith(
              createdAt: DateTime.now(),
              updatedAt: DateTime.now(),
      ...
      for (int i = 0; i < updatedLinks.length; i++) {
        ...
        if (!newOrderMap.containsKey(link.id) &&
            !titleOrderMap.containsKey(link.title)) {
          // プリセットリンクでない場合
