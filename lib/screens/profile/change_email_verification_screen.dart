...
class _ChangeEmailVerificationScreenState
  ...
  Future<void> _finishWithReauth(String newEmail) async {
    ...
    try {
      ...
      if (mounted) ScaffoldMessenger.of(context).showSnackBar(const SnackBar(
        content: Text('通知の解除に失敗しました。通信を確認して再試行してください。'),
      ));
      return;
    }
  ...
  Future<void> _checkCompletion({required bool showLoading}) async {
    ...
    try {
      final result = await UserService.checkEmailChangeCompletion(
        _pendingEmail!,
      );
        ...
        case EmailChangeCheckResult.notCompleted:
          if (showLoading) {
            ScaffoldMessenger.of(context).showSnackBar(
              const SnackBar(
                content: Text('まだ変更が反映されていません。メール内のリンクを開いてから再度お試しください。'),
              ),
            );
          }
      ...
      if (showLoading && mounted) {
        ScaffoldMessenger.of(
          context,
        ).showSnackBar(SnackBar(content: Text('確認に失敗しました: $e')));
      }
    } finally {
      if (showLoading && mounted) {
