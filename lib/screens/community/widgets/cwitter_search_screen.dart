...
class _CwitterSearchScreenState extends ConsumerState<CwitterSearchScreen>
  ...
  Future<void> _runSearch() async {
    ...
    try {
      ...
      final users =
          (await CwitterService.searchUsersByCwitterId(
            query,
          )).where((user) => !hiddenUserIds.contains(user.authorId)).toList();

      final tagUsers =
          (await CwitterService.searchUsersByHashtag(
            query,
          )).where((user) => !hiddenUserIds.contains(user.authorId)).toList();
    ...
    return Scaffold(
      appBar: AppBar(
        ...
        bottom: TabBar(
          ...
          tabs: const [Tab(text: 'Cweet'), Tab(text: 'ID'), Tab(text: 'タグ')],
        ),
      ),
  ...
  Widget _buildUserResults(
    ...
    if (query.isEmpty) {
      return _buildHint(theme, colorScheme, '@ または ID の一部を入力（部分一致）');
    }
  ...
  Widget _buildHashtagResults(
    ...
    return hashtagsAsync.when(
      ...
      data:
          (registeredHashtags) => _buildHashtagResultsBody(
            theme,
  ...
  Widget _buildHint(ThemeData theme, ColorScheme colorScheme, String message) {
    return Center(
      child: Padding(
        padding: const EdgeInsets.all(24),
...
class _HashtagChip extends StatelessWidget {
  const _HashtagChip({required this.summary, required this.onTap});
