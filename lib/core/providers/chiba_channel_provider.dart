...
final chibaChannelThreadsProvider = StreamProvider<List<ChibaChannelThread>>((
  ref,
) {
...
final chibaChannelFeedProvider = Provider<AsyncValue<ChibaChannelFeed>>((ref) {
  ...
  return ref
      .watch(chibaChannelThreadsProvider)
      .whenData(
        (threads) =>
            _splitThreads(threads, sortKey: sortKey, ascending: ascending),
      );
});
...
int _compareThreads(
  ...
  final result = switch (sortKey) {
    ChibaChannelThreadSortKey.lastActivity => a.lastActivityAt.compareTo(
      b.lastActivityAt,
    ),
    ChibaChannelThreadSortKey.createdAt => a.createdAt.compareTo(b.createdAt),
    ChibaChannelThreadSortKey.commentCount => a.commentCount.compareTo(
      b.commentCount,
    ),
  };
...
ChibaChannelFeed _splitThreads(
  ...
  final archived =
      threads.where((t) => t.isArchived).toList()
        ..sort((a, b) => b.lastActivityAt.compareTo(a.lastActivityAt));

  final active = threads.where((t) => !t.isArchived).toList();

  final hot =
      active.where((t) => t.isHot).toList()..sort((a, b) {
