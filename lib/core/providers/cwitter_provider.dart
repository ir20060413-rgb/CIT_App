...
class CwitterFeedNotifier extends StateNotifier<CwitterFeedState> {
  ...
  Future<void> refresh() async {
    ...
    try {
      ...
      state = CwitterFeedState(posts: page.posts, hasMore: page.hasMore);
    } catch (error) {
...
final cwitterUserBioProvider = StreamProvider.family<String?, String>((
  ref,
  userId,
) {
...
final _cwitterUserTagsStreamProvider =
    StreamProvider.family<List<String>, String>((ref, userId) {
      return UserService.watchUser(
        userId,
      ).map((user) => user?.cwitterTags ?? const []);
    });
...
Future<void> saveCwitterTags(
  ...
  ref
      .read(cwitterTagsOverrideProvider.notifier)
      .apply(userId: uid, tags: normalized);
...
final cwitterUserSocialLinksProvider =
    StreamProvider.family<Map<String, String>, String>((ref, userId) {
      return UserService.watchUser(
        userId,
      ).map((user) => user?.cwitterSocialLinks ?? const {});
    });
...
void markCwitterFeedSeen(WidgetRef ref) {
  final latestPostMs =
      ref
          .read(cwitterLatestPostCreatedAtProvider)
          .valueOrNull
  ...
  ref
      ...
      .setInt(cwitterLastSeenAtKey, latestPostMs);
...
final cwitterUserActivityProvider =
    StreamProvider.family<List<CwitterProfileActivity>, String>((
      ref,
      authorId,
    ) {
...
final filteredCwitterPostsProvider = Provider<AsyncValue<List<CwitterPost>>>((
  ref,
) {
...
final cwitterIsRecweetedProvider =
    StreamProvider.family<bool, ({String userId, String postId})>((
      ref,
      target,
    ) {
...
/// 指定 Cweet の返信数
final cwitterPostReplyCountProvider = StreamProvider.family<int, String>((
  ref,
  postId,
) {
...
final cwitterRankingsProvider = FutureProvider<CwitterRankingBoard>((
  ref,
) async {
  final hiddenUserIds =
  ...
  return CwitterRankingBoard(
    allTime: CwitterRankings(
      cweetCount: _filterRankingEntries(
        board.allTime.cweetCount,
        hiddenUserIds,
      ),
      followerCount: _filterRankingEntries(
        board.allTime.followerCount,
        hiddenUserIds,
      ),
    ...
    monthly: CwitterRankings(
      cweetCount: _filterRankingEntries(
        board.monthly.cweetCount,
        hiddenUserIds,
      ),
      followerCount: _filterRankingEntries(
        board.monthly.followerCount,
        hiddenUserIds,
      ),
